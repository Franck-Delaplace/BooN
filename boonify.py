...
# Fill the table.
        for stable in stablestates:
            ...
            for var in variables:
                ...
                # Z3 and BooN create Sympy symbols independently, so the same variable (e.g. 'FOS')
                # may be represented by two different Python objects in memory (a is b → False), even though
                # they are mathematically equal (a == b → True). This causes a KeyError when using stable[var]
                # directly. With small networks, Sympy's internal cache ensures the same object is reused,
                # so the KeyError never appears. With large networks (36+ variables), the cache is exceeded
                # and new objects are created, breaking direct dictionary lookup.
                # .get() solves this by trying first with the Sympy symbol, then with its string representation,
                # and finally returning None for free variables (e.g. PTEN = PTEN) that Z3 may omit from the model.
                # This is a workaround for the issue and does not affect the correctness of the stable states, as the variable names are consistent.
                # The stable states are generated by Z3 and converted to BooN, so the variable names in the stable states should match those in the BooN, allowing for successful lookup with .get().

                val = stable.get(var, stable.get(str(var), None))


                icon = QIcon()
                icon.addPixmap(QtGui.QPixmap(ICON01[val]), QtGui.QIcon.Normal, QtGui.QIcon.Off)
